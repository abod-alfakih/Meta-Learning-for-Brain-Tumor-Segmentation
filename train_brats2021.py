...
def main():
    ...
    load_pretrained_model(args, model, logger)
    # train & val
    ...
    for epoch in range(args.epochs):
        ...
        # clean_grads = []
        clean_grads = None
        number_batches = len(clean_train_loader)
        for j, (meta_inputs, meta_labels, _, _) in enumerate(clean_train_loader):
            ...
            if grads_model_2 is not None:
                weight = (j + 1) / number_batches  # Linear scaling from 0 to 1
                weighted_grads = grads_model_2 * weight
                if clean_grads is None:
                    clean_grads = weighted_grads.clone().detach()
                else:
                    clean_grads += weighted_grads
            # if grads_model_2 is not None:
            #     if clean_grads is None:
            #         clean_grads = grads_model_2.clone().detach()
            #     else:
            #         clean_grads += grads_model_2
        # After the loop, average the gradients
        clean_grads /= number_batches
        ...
        for i, (image, label, _, _) in enumerate(train_loader):
            ...
            grads_model = get_gradients(model)
            if grads_model is not None:
                similarity = cosine_similarity(grads_model, clean_grads)
                normalized_similarity = (similarity + 1) / 2  # Normalize from [-1, 1] to [0, 1]
                total_loss = loss * normalized_similarity
    ...
    infer(args, 86, model,loss_fn, val_loader, writer, logger, mode='test', save_pred=args.save_pred)
...
#             checkpoint = torch.load(args.weight_path)
#
#             # Check if the model is wrapped in DataParallel and extract the state dictionary
#             if isinstance(checkpoint['model'], torch.nn.DataParallel):
#                 model_state = checkpoint['model'].module.state_dict()
#                 print(1)
#             else:
#                 model_state = checkpoint['model']
#                 print(2)
